...
class Moderation(commands.Cog):
    ...
    async def unmute(self, ctx, member: discord.Member):
        await member.remove_roles(self.mutedrole)
        await ctx.send(f"{ctx.author.name} размучен {member.display_name}")
        mutes = load_json("jsons/mutes.json")
        mutes.pop(str(member.id))
        write_json("jsons/mutes.json", mutes)
    ...
    async def on_ready(self):
        self.guild = await self.bot.fetch_guild('979117594223198238')
    ...
    async def ban_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.send('У вас не достаточно прав для того чтобы использовать данную команду!', delete_after=5)
            return
        if isinstance(error, commands.MemberNotFound):
            await ctx.message.delete()
            await ctx.send('Пользователь не был найден!', delete_after=5)
            return
    ...
    async def unban_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            await ctx.message.delete()
            await ctx.send('У вас не достаточно прав для использование данной команды', delete_after=5)
def setup(bot):
